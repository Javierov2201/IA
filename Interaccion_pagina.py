...
def insertar_datos(uuee, expediente, datos_expediente) :
    conn = sqlite3.connect("MEF.db")
    cur = conn.cursor()
    cur.executemany('INSERT OR IGNORE INTO UUEE VALUES (?,?)', uuee)
    cur.executemany('INSERT OR IGNORE INTO Expediente VALUES (?,?,?,?,?,?,?,?,?)', expediente)
    cur.executemany('INSERT OR IGNORE INTO DatosExpediente VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)', datos_expediente)
    conn.commit()
    cur.close()
    conn.close()
